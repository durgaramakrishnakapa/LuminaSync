...
def index_exists(file_path, index_name):
    """
    Checks if the index file already exists for the given document.
    ...
    index_file = f"{index_name}_{file_hash}.index"
...
def run_rag_app(file_path, text_query):
    ...
    if index_exists(file_path, index_name):
        print(f"Index already exists for {file_path}. Skipping re-indexing.")
    else:
